...
function returnEmoji(mood){
  if(mood < 0.33){
    return emojione.toImage(':worried:').replace('//','https://');
  }else if(mood < 0.66){
    return emojione.toImage(':sweat:').replace('//','https://');
  }else {
    return emojione.toImage(':smile:').replace('//','https://');
  }
}
loopApp.controller('HomeController', ['$scope', '$firebaseArray', '$rootScope', '$sce', function($scope, $firebaseArray, $rootScope, $sce) {
  ...
  $scope.articles.$loaded().then(function(){
    ...
    for(var i = 0; i < total; i++){
      ...
      $('<img src="http://beintheloop.me/img/uploads/'+$scope.articles[i].image+'" />').load(function(){
        loaded++;
